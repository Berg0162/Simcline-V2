...
During operation Zwift sends (about every 800 ms) new settings, and one of these is the grade value (road inclination in degrees). Simcline-V2 translates the grade to a level that should be reached by the actuator to simulate exactly the road grade that was received from Zwift. However, the actuator can only be switched to <b>move up</b>, <b>move down</b> or <b>stop</b>. After having set the actuator to move (up or down), the program has to check continuously if the actuator has reached the desired level by reading its position with the help of the Time-Of-Flight sensor and act accordingly. To avoid conflicts during variable updates (i.c. TargetPosition) a Binary Semaphore scheme is applied to protect <b>task shared variables</b> during an update and presentation. <br>
Meanwhile the trainer sends all your cycling data (CPS, CSC, FTMS and optional HRM) 4-5 times per second over BLE MITM channels. The Zwift app has to confirm the receipt of several of these data. Vice versa, the (virtual ride) data sent by Zwift have to be tranferred to the trainer and also the trainer has to confirm the receipt of several of these data. Simcline-V2, being a BLE MITM, handles a huge amount of BLE traffic per second and it does not allow for mistakes! <br>
The load of Simcline-V2 is balanced over 2 processor cores of the ESP32 processor: <br>
<b>On Core 0</b>: exclusively all the BLE handling (NimBLE-Arduino) <br>
<b>On Core 1</b>: the critical control of the actuator, presentation (display) and other small tasks. <br>
