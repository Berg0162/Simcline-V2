...
Today <b>Bluetooth Smart FTMS</b> is the absolute industry standard for indoor trainers and Simcline-V2 is primarily targeted at <b>FTMS</b>! However only since 2020 most trainers have builtin support for FTMS. Simcline-V2 has now builtin support for **Tacx Smart Trainers with (ANT+) FE-C over BLE** to meet the needs of early adapters of SIMCLINE!<br>
...
Since about 2015 Tacx produced Tacx Smart Trainers that comply **ANT+ FE-C** and **Bluetooth**. Tacx implemented a proprietary BLE-protocol then  called: '(ANT+) FE-C over BLE'. Tacx designed this BLE protocol because at that time an open standard on BLE for trainers was lacking. 
You think this applies to your legacy Tacx trainer? Always check first if your legacy Tacx trainer has support for the (ANT+) FE-C protocol over BLE, since NOT all legacy Tacx Smart Trainers that are labelled 'smart' are equally 'smart'!

This functionality support is now builtin with Simcline-V2, you only have to **activate** it, when using a legacy Tacx smart trainer! Notice that FTMS and legacy Tacx FE-C mutually exclude each other within Simcline-V2!<br>
